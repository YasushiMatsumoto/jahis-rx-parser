...
- Ver.2.5 で追加された `31` レコード、および `201` の一般名関連項目は未完了または一部未対応。
- Ver.2.4 で追加された `421` レコードは未対応。
...
| `2`        | 患者特記レコード                               | Supported            | Yes     | Yes       | Yes      | Indirect | 種別 `1/2/3/9` とレコード作成者を検証。 |
| `3`        | 要指導医薬品・一般用医薬品服用レコード         | Partially Supported  | Partial | Partial   | Partial  | Indirect | 名称、開始日、終了日、作成者は保持。通番と JAN コードは未対応。 |
| `31`       | 要指導医薬品・一般用医薬品成分レコード         | Not Yet Supported    | No      | No        | No       | No   | 未実装。未知レコードとして保持。 |
| `4`        | 手帳メモレコード                               | Supported            | Yes     | Yes       | Yes      | Indirect | メモ本文、入力日、作成者を保持。 |
| `5`        | 調剤等年月日レコード                           | Supported            | Yes     | Yes       | Yes      | Yes  | 調剤ブロック開始として扱う。 |
| `11`       | 調剤－医療機関等レコード                       | Supported            | Yes     | Yes       | Yes      | Yes  | 出力区分 `1` では都道府県、点数表、コードを必須検証。 |
| `15`       | 調剤－医師・薬剤師レコード                     | Supported            | Yes     | Yes       | Yes      | Indirect | 氏名、連絡先、作成者を保持。 |
| `51`       | 処方－医療機関レコード                         | Supported            | Yes     | Yes       | Yes      | Yes  | 調剤ブロック内に薬品系レコードがある場合の必須条件を検証。 |
| `55`       | 処方－医師レコード                             | Supported            | Yes     | Yes       | Yes      | Indirect | 複数医師レコードの繰り返しを保持可能。 |
| `201`      | 薬品レコード                                   | Partially Supported  | Partial | Partial   | Partial  | Yes  | 主項目は対応。一般名、一般名コード種別、一般名コードは未保持。 |
...
| `421`      | 残薬確認レコード                               | Not Yet Supported    | No      | No        | No       | No   | 未実装。未知レコードとして保持。 |
...
- `31` 要指導医薬品・一般用医薬品成分レコードの decode/normalize/validate 実装。
- `421` 残薬確認レコードの decode/normalize/validate 実装。
- `201` の一般名・一般名コード関連項目の保持。
- `3` の通番・JAN コードの保持。
- 仕様書 Ver.2.6 に合わせたヘッダ版番号 `JAHISTC08` 固定化の要否検討。
